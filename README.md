...
Funciona en Linux, macOS, Windows, Termux y Heroku con Node.js 20 o superior. El repositorio incluye un worker de Heroku para ejecutar el bot sin reemplazar la instalación local.
...
- Una aplicación de Heroku con un dyno `worker` si se elige ese despliegue.
...
| `!menu` | Alias de `!ayuda`. |
| `!entretenimiento` / `!juegos` | Muestra juegos y utilidades. |
| `!dado [caras]` | Lanza un dado de 2 a 1000 caras. |
| `!moneda` | Lanza una moneda. |
| `!8ball <pregunta>` | Responde una pregunta de forma lúdica. |
| `!verdad` / `!reto` | Propone una pregunta o reto seguro. |
| `!pareja <nombre 1> y <nombre 2>` | Calcula compatibilidad aleatoria como entretenimiento. |
...
En Heroku, el directorio `auth_info/` vive en un sistema de archivos efímero. Un reinicio, redeploy o cambio de dyno puede exigir volver a vincular WhatsApp. `Procfile` define el proceso `worker` y `app.json` contiene la configuración de despliegue sin secretos.

## Pruebas y scripts

Ejecuta la suite de **26 pruebas** con:
...
- Si ejecutas el bot en Heroku, no dependas de archivos locales para conservar la sesión o la lista de vigilancia; realiza copias seguras o vuelve a vincular cuando el dyno se reinicie.
- Mantén `CONTROL_API_TOKEN` vacío en Heroku salvo que hayas añadido una protección de red y autenticación apropiadas; el Control API está diseñado principalmente para una red local.
